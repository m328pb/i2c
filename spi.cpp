...
void SPI_init(uint8_t clock_div) {
  ...
  SPCR0 = ((0 & ~(1 << SPIE)) | // disable interrupts SPIE=0
           (1 << SPE) |         // SPI Enable =1,
           (1 << MSTR)) &       // set Master mode MSTR=1,
          ~(1 << DORD) &        // Data order:  MSB first when DORD=0
          ~(1 << CPOL) & // SPI mode: CPOL=0: lead edge - Rising; trailing edge:
                         // Falling
  ...
  SPSR0 = (SPSR0 & ~(1 << SPI2X)) | (div & 0b100);
  SPCR0 = (SPCR0 & ~(1 << SPR1)) | (div & 0b010);
  SPCR0 = (SPCR0 & ~(1 << SPR0)) | (div & 0b001);
}
